...
A privacy-preserving sealed-bid auction dApp on [Midnight](https://midnight.network/), built with
zero-knowledge proofs on testnet.

[![CI](https://github.com/swarupasaha2005-hue/SecretBid/workflows/CI/badge.svg)](https://github.com/swarupasaha2005-hue/SecretBid/actions/workflows/ci.yaml)
...
**Live Demo:** [midnight3.vercel.app](https://midnight3.vercel.app) ([alternate link](https://midnight3-215s6bnnj-swarupasaha2005-hues-projects.vercel.app/))

> **Use this repo as a template — do not fork it.** This repository is intended to be used via
> GitHub's "Use this template" flow. Forks are not tracked as independent projects.

## Contract Address

> **Required before submission.** Replace the placeholder below with the real address printed by
> the CLI after deployment — see [Manual Deployment](#manual-deployment) and
...
Traditional on-chain auctions leak every bid to everyone the moment it's submitted — other bidders,
and even the auction creator, can see and react to bid amounts before the auction closes.

SecretBid fixes this with zero-knowledge proofs. A bidder commits to a bid amount via a
...
- **Reveal bids securely** — `revealBid` discloses the real amount and proves it matches the
  original commitment.
- **Winner determination** — `revealBid` incrementally tracks the leading bid as each bidder
  reveals (highest revealed bid that meets the reserve price wins); `closeAuction` finalizes the
  auction once every commitment has been revealed.
...
All five circuits (`createAuction`, `commitBid`, `startReveal`, `revealBid`, `closeAuction`) are
production-complete: each enforces the relevant phase checks, authorization (only an auction's
creator may call `startReveal`/`closeAuction`), commitment creation and verification, and
incremental winner selection, in addition to its ledger mutations. See the doc comments in
`secretbid.compact` for the full validation and behavior of each circuit.
...
As noted in the contract's own doc comments, every circuit fully enforces its business rules —
phase checks, authorization, commitment verification, and winner selection — in addition to its
ledger mutations. See [Known Limitations](#known-limitations) for the one remaining protocol-level
constraint (every committed bidder must reveal before an auction can close).
...
Each action creates a real transaction on Midnight Testnet using zero-knowledge proofs generated by the proof server. The circuits behind these actions are production-complete: they enforce the real auction rules (phase checks, authorization, commitment verification, and winner selection) in addition to the ledger wiring (creating records, storing commitments, transitioning phases) — see `contract/src/secretbid.compact` for the full documented behavior of each circuit.
...
4. Use the SecretBid web interface
...
(`closeAuction` asserts `revealCount == commitCount`) — there is no way to close around a bidder who
commits and never reveals. Timeout-based recovery is planned for a future version; see
...
|---|---|
| `npm install` fails | Ensure you're using Node `v24.11.1` or newer. Older Node versions may install with warnings but are not the target runtime. |
| Contract compilation fails | Ensure the Compact toolchain is installed and run `npm run compact` from `contract/`. |
| Network connection timeout | The CLI requires an internet connection — restart it if the connection times out. |
| Token funding takes too long | Wait 1–2 minutes; funding is automatic in the CLI. |
| "Application not authorized" error | Start the proof server: `docker compose -f proof-server-local.yml up -d`. |
| Lace wallet not detected | Install the Lace wallet browser extension and refresh the page. |
| Docker issues | Ensure Docker Desktop is running — check with `docker --version`. |
| Port 6300 in use | Run `docker compose down`, then restart the services. |
| Dependencies won't install | Use a Node.js LTS version. For older npm versions, you may need `--legacy-peer-deps`. |
| Contract deployment fails | Verify the wallet has sufficient balance and a working network connection. |
...
Licensed under the [Apache License 2.0](./LICENSE) — see the license header at the top of every
